...
jQuery(document).ready(function ($) {
	...
	if (window.location.href == getAppURL('sales/add')) {
		...
		document.getElementById("delivery_note").style.visibility = "hidden";
		...
		$('#trans_items_table tbody tr').each(function (e) {
			var id = $(this).attr('data-index');
			var wname = 'transItems[' + id + '][warehouses]';
			var sname = 'transItems[' + id + '][shelfs]';
			$(this).find('td input').attr('readonly', true);
			var warehouse = $(this).find('td select.i-warehouse').val();
			var shelf = $(this).find('td select.i-shelf').val();
			$(this).find('td select.i-warehouse').replaceWith('<input type="text" name="' + wname + '" class="form-control i-warehouse" value="' + warehouse + '" style="width:100px;" readonly>');
			$(this).find('td select.i-shelf').replaceWith('<input type="text" name="' + sname + '" class="form-control i-warehouse" value="' + shelf + '" style="width:100px;" readonly>');
		});
	}
	...
	if ($('#status').is(':checked') === false) {
		...
		if ($('#payment_method').val() === "Cash" || $('#payment_method').val() === " ") {
			$('#trans_nb_div').hide();
		}
	}
					...
					$.ajax({
						...
						success: function (data) {
							...
							jQuery('tr', $transactionLines).each(function (i, tr) {
								...
								//profit
								var cost1 = cost / parseFloat(currency_rate);
								let net_price = (parseFloat(price) * (1 - (jQuery('input.i-discount', tr).val() / 100)));
								// var profit = parseFloat(net_price) - parseFloat(cost1);
								profit = (((parseFloat(jQuery('input.i-price', tr).val()) - parseFloat(jQuery('input.i-cost', tr).val())) * (1 - (parseFloat(jQuery('input.i-discount', tr).val()) / 100))) * 100) / (parseFloat(jQuery('input.i-cost', tr).val()));
...
function addMissingItemLineToTransaction(item, newLine) {
	...
	if (newLine) {
		...
		var cost = parseFloat(item.cost) / parseFloat(currency_rate);
		var profit = (((parseFloat(jQuery('input.i-price', $tr).val()) - parseFloat(jQuery('input.i-cost', $tr).val())) * (1 - (parseFloat(jQuery('input.i-discount', $tr).val()) / 100))) * 100) / (parseFloat(jQuery('input.i-cost', $tr).val()));
		// var profit = parseFloat(price) - parseFloat(cost);
		jQuery('input.i-profit', $tr).val(profit.toFixed(2));
		jQuery('input.i-discount', $tr).val('0');
		jQuery('td.i-total', $tr).text(jQuery('input.i-price', $tr).val());
...
function bindLineEvents($tr) {
	...
	$('#transactionLines tr').click(function () {
		...
		$.ajax({
			...
			success: function (data) {
				...
				$("#activityitemTbl tr").on('click', function (e) {
					...
					var trans_item_id = $('#transactionLines tr#' + row_id).find('td').find("input[class='hide i-item_id']").val();
					...
					if (trans_item_id == activity_item_id) {
						...
						console.log(w, s)
						$('#transactionLines tr#' + row_id).find('td').eq(5).find('input').val(w);
						$('#transactionLines tr#' + row_id).find('td').eq(6).find('input').val(s);
					}
					// else {
					// 	$.ajax({
					// 		cache: false,
	...
	function calculateLineTotal() {
		...
		//profit
		var cost = parseFloat(jQuery('input.i-item_cost', $tr).val()) / parseFloat(currency_rate);
		let net_price = (parseFloat(jQuery('input.i-price', $tr).val()) * (1 - ($discount.val() / 100)));
		// var profit = parseFloat(net_price).toFixed(2) - parseFloat(cost).toFixed(2);
		//var profit = (((parseFloat(jQuery('input.i-price', $tr).val()) - parseFloat(jQuery('input.i-cost', $tr).val())) * (1 - (parseFloat($discount.val()) / 100))) * 100) / (parseFloat(jQuery('input.i-cost', $tr).val()));
		profit = parseFloat(total.toFixed(2)) - (parseFloat(jQuery('input.i-cost', $tr).val()) * parseFloat($qty.val()));
...
function validation() {
	...
	jQuery('tr', $transactionLines).each(function (i, tr) {
		...
		// 		type: 'POST',
		// 		async: false,
		// 		data: { 'qty': qty, 'shelf': shelf, 'warehouse': warehouse, 'item_id': item_id, 'trans_id': $('#id').val() },
		// 		url: getAppURL('items/get_max_qty_for_each_warehouse_shelf_for_edit_sale'),
		// 		success: function (data) {
		// 			// alert(data);
		// 			if (parseFloat(qty) > parseFloat(data) || parseFloat(qty) < 1) {
		// 				error = jQuery('div.i-error_qty', tr);
...
function itemsAvailabilitytable() {
	...
	if (itemid[0] !== undefined) {
			...
			success: function (data) {
				...
				for (let j = 0; j < data.length; j++) {
					for (let i = 0; i < data[j].length; i++) {
						...
						$("#activityitemTbl tbody").append(r);
						$("#activityitemTbl tbody").append("<td colspan='6' style='color:white'>t</td>");
					}
				}
			}
		});
		...
		$("#activityitemTbl td").remove();
		$("#activityitemTbl tbody").append("<td colspan='6' style='text-align: center;'>No Data Found</td>");
	}
}